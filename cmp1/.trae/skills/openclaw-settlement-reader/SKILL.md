...
## 功能
- 根据传入的文件夹目录自动查找 `外包结算单测试模版.xlsx` 和 `附件五` 文件
...
## 实现原理
1. 接收传入的文件夹目录参数（或使用当前目录）
2. 在指定文件夹中自动查找 `外包结算单测试模版.xlsx` 和 `附件五` 文件
3. 打开找到的 `外包结算单测试模版.xlsx` 文件
4. 遍历所有工作表
5. 在每个工作表中查找内容为 `钉钉编号` 的单元格
6. 提取该单元格及其后续的所有数据
7. 读取附件五文件中的模块名称和单价数据
8. 比较附件五数据和提取数据，根据比较结果添加颜色标记
9. 生成模板文件（只保留标题行）
10. 将附件五的红河/文山和普洱数据依次填入模板
11. 在区域列填入具体的县名
12. 通过三列匹配（区域、施工内容、单价）来填充施工量
13. 根据10个计算公式自动计算所有相关金额列
14. 删除不需要的列（比较结果、附件五单价）
15. 将提取的数据和生成的模板保存到指定文件夹的新Excel文件中
...
# 使用指定目录
python read_settlement.py /path/to/folder
```

### 程序调用

```python
from read_settlement import main
...
## 注意事项
- 确保文件夹中包含 `外包结算单测试模版.xlsx` 文件
- 确保文件夹中包含 `附件五` 文件（文件名包含"附件五"字样）
