...
def compare_data(extracted_data, attachment_data):
    ...
    # 不添加缺失数据，只保留有钉钉编号的行
    # 添加缺失数据
    # existing_contents = set(str(content).strip() for content in comparison_data['施工内容'])
    # 
    # # 查找缺失的数据
    # missing_data = []
    # for _, row in attachment_data.iterrows():
    ...
    #         # 创建一行空数据，只填充施工内容和单价
    #         missing_row = {col: '' for col in comparison_data.columns}
    #         missing_row['施工内容'] = content
    #         missing_row['单项结算金额单价'] = row['单项结算金额单价']
    #         missing_row['附件五单价'] = row['单项结算金额单价']
    #         missing_row['比较结果'] = '不存在'
    #         missing_data.append(missing_row)
    # 
    # if missing_data:
    #     print(f"发现 {len(missing_data)} 条缺失数据，添加到结果末尾")
    #     missing_df = pd.DataFrame(missing_data)
    #     comparison_data = pd.concat([comparison_data, missing_df], ignore_index=True)
    # else:
    #     print("没有缺失数据")
